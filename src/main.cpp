		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2')
		{
			...
			if (s != "")
			{
				...
				if (event == "telemetry")
				{
					...
					if (prev_size < 2)
					{
						// use two points that makes path tagent to the car
						double prev_car_x = car_x - cos(car_yaw); // is car_yaw in radian?
					...
					// use prev path's end point as starting reference
					else
					{
					...
					// find 30m, 60m, 90m space points in the frenet reference system
					...
					{
						// shfit car reference angle to 0 degree
					...
					{
						double N = (target_dist / (0.02 * ref_vel / 2.24)); // "2.24" is translate mph to meter-per-s
